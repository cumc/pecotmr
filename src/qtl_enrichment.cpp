    ...
    double prior_variance) 
{
    ...
    for (int k = 0; k < ImpN; k++) {
        std::vector<int> qtl_sample(gwas_pip.size(), 0.0);
...
std::map<std::string, double> output = qtl_enrichment_workhorse(susie_fits, gwas_pip, gwas_pip_names, pi_gwas, pi_eqtl, ImpN, prior_variance);
}
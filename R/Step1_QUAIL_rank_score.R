...
calculate_integrated_score <- function(rank_scores, method = "equal", num_tau_levels) {
  if (method == "equal") {
    ...
    for (i in 1:num_tau_levels) {
      weight <- ifelse((i > (num_tau_levels) / 2), 1, -1)
      int_rank_score <- int_rank_score + weight * rank_scores[[i]]
    }
  } else if (method == "ivw") {
    # IVW method implementation
    a_i_tau_diff_matrix <- matrix(0, 
                                nrow = length(rank_scores[[1]]), 
                                ncol = num_tau_levels / 2)
    for (i in 1:(num_tau_levels / 2)) {
      a_i_tau_diff_matrix[, i] <- rank_scores[[i + num_tau_levels / 2]] - rank_scores[[i]]
    }
    
    # Optimization steps
    Y_QI_Var_Cov <- cov(a_i_tau_diff_matrix)
    Dmat <- Y_QI_Var_Cov
    dvec <- rep(0, num_tau_levels / 2)
    Amat <- cbind(rep(1, num_tau_levels / 2), diag(num_tau_levels / 2))
    bvec <- c(1, rep(0, num_tau_levels / 2))
    ...
    for (i in 1:(num_tau_levels / 2)) {
      int_rank_score <- int_rank_score + weights_vec[i] * 
        (rank_scores[[i + num_tau_levels / 2]] - rank_scores[[i]])
    }
  }
  
  return(int_rank_score / (num_tau_levels / 2))
}

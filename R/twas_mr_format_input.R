...
######generate the pvalue and qvalue of each gene
#twas_z_format: the output of ptwas.R function
twas_pq  = function(twas_z_format){
  ...
  twas_weights = data.frame(gene_name=weights$gene_name,chr = weights$chr,weights$susie_weights$pval,weights$susie_weights$z,
  ...
  qvalue = apply(twas_weights[,c("susie_pval","lasso_pval","enet_pval","mr_ash_pval")],2,function(x) qvalue(x,lambda=0)$qvalues)
  gene_weights_pq = data.frame(twas_weights,qvalue = qvalue)
  names(gene_weights_pq)[11:14]=c("susie_qval","lasso_qval","enet_qval","mr_ash_qval")
  return(gene_weights_pq)
}
...
twas_mr_format_input = function(cand_genes,susie_res_path,AD_GWAS_path){
  ...
  if(!is.null(names(qtl_susie_res[[1]]$sets$cs))){
     qtl_finemap = qtl_susie_res[[1]]$top_loci%>%filter(cs_index_primary>=1)%>%
          select(-cs_index_secondary)%>%mutate(X_ID=gene_name)%>%
...
##transform the AD_dataset to the form of allele_qc format
     AD_data = AD_dataset%>%mutate(chr = paste0("chr",chromosome))%>%mutate(pos = as.character(position))%>%
          select(-chromosome)%>%rename("A1"="ref","A2"="alt")%>%mutate(z=beta/se)%>%select(-position)
     ...
     format_input = qtl_finemap%>%
          cbind(.,AD_allele_flip[match(qtl_finemap$snp,AD_allele_flip$variant_allele_flip),]%>%
          select(beta,se)%>%
